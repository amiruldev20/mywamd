...
let conn = (set.opts['legacy'] ? makeWALegacySocket : _makeWaSocket)(connectionOptions)
...
// send vn
sendVn: {
async value(jid, url, quoted) {
var send = await conn.sendMessage(jid, { audio: await (await fetch(url)).buffer(), ptt: true, mimetype: 'audio/mpeg' }, { quoted: quoted })
return send
}
},
...
// reply with thumb
repImg: {
async value (jid, text, men, quoted){
return conn.sendMessage(jid, {
text: text,
jpegThumbnail: await (await fetch(set.tumb)).buffer(),
mentions: [men],
contextInfo: {
isForwarded: true,
 ...
 * @param {*} options 
 */
 async value(jid, to, inviteCode, inviteExpiration, groupName, caption, jpegThumbnail) {
...
const store2 = store.makeInMemoryStore()
try {
store2.bind(sock.ev, {
groupMetadata: sock.groupMetadata
})
var nadia = `${set.opts._[0] || 'renz'}.store.json`
setInterval(() => {
store2.writeToFile(nadia)
}, 60 * 1000)
} catch {
console.log("[WABOTJS] CAN'T WRITE STORE")
}
